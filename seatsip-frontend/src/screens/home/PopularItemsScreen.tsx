...
export default function PopularItemsScreen() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      setLoading(true);
      const res = await cafesApi.getPopularItems();
      setItems(res.data.data || []);
    } catch (err) {
      console.log('Error fetching popular items:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const renderItem = ({ item }: { item: any }) => {
    return (
      <View style={styles.itemCard}>
        <Image source={{ uri: item.image_url || 'https://images.unsplash.com/photo-1572442388796-11668a67e53d?w=300' }} style={styles.itemImg} />
        <View style={styles.itemInfo}>
          ...
          <Text style={styles.cafeName} numberOfLines={1}>at {item.cafe_name}</Text>
          ...
          <View style={styles.priceRow}>
            ...
            <TouchableOpacity 
              style={styles.addBtn}
              onPress={() => {
                // For global popular items, we need cafe_id to add to cart
                // addToCart logic usually requires current cafe context
                // For now, navigate to cafe menu or handle it
                navigation.navigate('Menu', { cafeId: item.cafe_id, cafeName: item.cafe_name });
              }}
            >
      ...
      <SafeAreaView style={{ flex: 1, width: '100%' }}>
        ...
        {/* {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.centered}>
                <Text style={styles.emptyTxt}>No popular items found right now.</Text>
              </View>
            }
          />
        {/* )} */}
